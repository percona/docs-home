...
[Percona Server for MySQL 5.7.44-57](https://docs.percona.com/percona-server/5.7/){:target="_blank"} has been released on Monday, March 9, 2026.
...
* Queries with thousands of joins spend significant time in `add_table_to_list()` and resolver before failing with `ER_TOO_MANY_TABLES`. Example: 20k-way join parses all 20k tables before error.

* Fixes a memory leak with prefix index on blob columns.

Learn more in Percona Server for MySQL 5.7.44-57 [release notes](https://docs.percona.com/percona-server/5.7/release-notes/5.7.44-57.html){:target="_blank"}.
