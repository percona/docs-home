...
date: 2026-09-10
description: >
  Percona XtraDB Cluster 9.7.1-1 has been released on September 10, 2026.
...
# Percona XtraDB Cluster 9.7.1-1 has been released

<!-- more -->

[Percona XtraDB Cluster 9.7.1-1](https://docs.percona.com/percona-xtradb-cluster/9.7/){:target="_blank"} has been released on September 10, 2026.

Try it out using the [Quickstart guide](https://docs.percona.com/percona-xtradb-cluster/9.7/quickstart-overview.html){:target="_blank"}.
...
This release is based on Percona Server for MySQL 9.7.1-1.
...
* [CVE-2026-48165](https://www.cve.org/CVERecord?id=CVE-2026-48165): A vulnerability in the handling of the `wsrep_sst_donor` and `wsrep_sst_receive_address` system variables could allow a highly privileged user to execute arbitrary shell commands with the privileges of the `mysqld` process during State Snapshot Transfer (SST) due to improper input sanitization (CVSS 3.1 Base Score 7.2). The fix was backported from MariaDB. Thanks to the MariaDB project for identifying and fixing this issue.

* [CVE-2026-49261](https://www.cve.org/CVERecord?id=CVE-2026-49261): A vulnerability in `wsrep_notify_cmd` could allow a remote attacker to execute arbitrary shell commands by embedding commands in the name of a joining node when `wsrep_notify_cmd` is enabled due to improper input sanitization (CVSS 3.1 Base Score 10.0). The fix was backported from MariaDB. Thanks to the MariaDB project for identifying and fixing this issue.
